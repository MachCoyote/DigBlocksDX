...
Open the Bootstrap scene, enter Play Mode, and choose Play. The authoritative development source preserves the original nine fixture chunks and supplies bounded sample Perlin chunks through the existing replication channel as moving interest requires them. The default independent render distances admit 75 chunks; see the [dynamic chunk loading implementation summary](dynamic-chunk-loading.md). The inspection camera uses WASD, mouse look, Space / Left Control for elevation, and Left Shift for speed. Menu focus gates camera input. A Scene view open during Play Mode shows the same terrain and can be flown independently. The bootstrap `useTerrainFixture` flag disables this bounded source; it is not final world generation or persistence.
...
`Client/Rendering/ChunkMeshScheduler.cs` observes replica publication, individual removal, and reset events and schedules bounded workers from a distance-and-age priority queue. A revision or removal conservatively invalidates the chunk and its six neighbors. Individual removal retires only the departed GPU slot, while reset clears all geometry and graph state. Geometry and connectivity handles are combined before publication, which checks store identity, interest epoch, entry version, and all seven presence/incarnation/revision stamps. Empty output removes geometry but still publishes a resident graph node. Stale jobs finish safely and cannot publish. Readiness requires every admitted chunk's latest dirty version to have matching geometry and connectivity.

`TerrainRenderer.cs` owns long-lived GPU resources. `TerrainRenderService.cs` binds it to the client ECS world and session lifetime and currently reports the fly camera's chunk as the temporary local-player anchor. Dedicated servers create no renderer. The world-ready gate waits for authoritative replica data before visual readiness, preventing an initial empty store from being treated as playable terrain.
...
Mapped staging is copied into device geometry by compute. Replacement ranges are reference-counted by both uploads and frame snapshots. Retired ranges become reusable only after all owners release them. An asynchronous readback is the completion token wherever the device supports one, because an async-queue fence cannot be polled on every backend; a fence is used only where readback is unavailable. A token may be re-inserted before an earlier request lands, so completion means every outstanding request finished rather than whichever returned first. Ordinary streaming does not synchronously wait for the GPU. Session teardown drains the last submission before disposal.

### Completion tokens must not be queued into the render context

A completion token has to be issued with `Graphics.ExecuteCommandBuffer`, never with `ScriptableRenderContext.ExecuteCommandBuffer`. `TerrainRenderer.EndCamera` runs from `RenderPipelineManager.endCameraRendering`, by which point URP has already submitted that camera's context. Work queued into the context there is never submitted again, so the token never reaches the GPU and the frame it belongs to never completes. This applies to both token kinds: a `GraphicsFence` never signals and an `AsyncGPUReadback` never delivers its callback.

A queued token is dropped permanently, not merely delayed: `TerrainCompletionProbe` measures a readback queued this way never completing even when a second camera renders and submits afterwards, while the same token issued through `Graphics.ExecuteCommandBuffer` completes with a single camera. Run `DigBlocks/Verification/Terrain completion token path` to reproduce.

Why the Editor tolerated this and a player did not is **not** established. An earlier explanation here, that an open Scene view flushed the queued token by submitting the context again, is disproven by that probe. The open candidate is that the Editor strands primary submissions often enough that `ResolveStrandedSubmissions` recycles the ring through `Graphics.ExecuteCommandBuffer` instead, which the standalone diagnostics support but do not prove: the build reported `strandIns=0` against roughly one `EndCamera` insert per frame, so in a player every frame took the broken path and none took the working one. The symptom is a frame ring that consumes its `FrameSlots` snapshots during the first few frames and then never recycles one. Meshing, streaming and uploads continue normally and readiness still completes, so the world loads in full while the camera keeps re-submitting the last visible set it managed to build. Terrain appears frozen a handful of chunks in, stays visible from angles it should be culled from, and shows gaps where faces were culled against the camera position held when the ring jammed.
