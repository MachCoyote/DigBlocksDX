...
The failure is invisible in the Editor and fatal in a player. An open Scene view submits the context again within the same render loop, which flushes whatever a previous camera's `endCameraRendering` left queued; a build drawing one camera never flushes it. The symptom is a frame ring that consumes its `FrameSlots` snapshots during the first few frames and then never recycles one. Meshing, streaming and uploads continue normally and readiness still completes, so the world loads in full while the camera keeps re-submitting the last visible set it managed to build. Terrain appears frozen a handful of chunks in, stays visible from angles it should be culled from, and shows gaps where faces were culled against the camera position held when the ring jammed.
